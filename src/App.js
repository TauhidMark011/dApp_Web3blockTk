...
class App extends Component {
	...
	componentDidMount = async () => {
		try {
			...
			// const Accounts = process.env.REACT_APP_ACCOUNT;
			const tauhidAccounts =
				"0x6b9A9B1FE3dAa61fbf29653FBb7deD98E1e1c70D" ||
				process.env.REACT_APP_ACCOUNT;
			// console.log(accounts);
			// console.log(tauhidAccounts);

			// Get the contract instance.
			const networkId = await web3.tauhid.eth.net.getId();
			// console.log("id" + networkId);
			const deployedNetwork = TokenGenerator.networks[networkId];
			const instance = new web3.tauhid.eth.Contract(
				TokenGenerator.abi,
			...
			this.setState({ web3, accounts, tauhidAccounts, contract: instance });
	...
	makeToken = async (event) => {
		...
		// );
		// console.log(this.state.tauhidAccounts + " " + this.state.accounts[0]);
		...
		// var nonce = (await this.state.web3.tauhid.eth.getTransactionCount(this.state.tauhidAccounts));
		// nonce += 16;
		// console.log(nonce);
		// console.log(this.state.contract.options.address);
		// var _nonce = nonce.toString(16);
		const tx = await this.state.web3.tauhid.eth.accounts.signTransaction(
			{
				// nonce:'0x' + _nonce,
		...
		// console.log(tx);
		// console.log(
		// 	await this.state.web3.tauhid.eth.accounts.recoverTransaction(
		// 		tx.rawTransaction
		// 	)
		// );

		const resp = await this.state.web3.tauhid.eth.sendSignedTransaction(
			tx.rawTransaction
		);
		console.log(resp);
